...
output_file_dir = "../results/coverage_dependency_experiment/"
...
plt.rcParams['lines.markersize'] = 7  # Increase marker size for all plots
...
for i in range(len(tool_names)):
    ...
    plt.plot(list(hamming_distances_dict.keys()), hamming_distances, label=tool_names[i], marker=markers[i], linewidth=2.5)
plt.xlabel("Coverage")
plt.ylabel("Average Hamming Distance")
plt.yscale("log")
plt.tight_layout()
plt.legend()
plt.savefig("../results/hamming_dist_vs_coverage.pdf")
plt.show()
...
for i in range(len(tool_names)):
    failure_rates = [1 - success_rates_dict[cluster_size][i] for cluster_size in success_rates_dict]
    plt.plot(list(success_rates_dict.keys()), failure_rates, label=tool_names[i], marker=markers[i], linewidth=2.5)
...
plt.tight_layout()
plt.legend()
plt.savefig("../results/failure_rate_vs_coverage.pdf")
plt.show()