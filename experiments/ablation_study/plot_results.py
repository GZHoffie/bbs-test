...
output_file_dir = "../results/ablation_study/"

tool_names = ["BBS DBG weight", "BBS Unidirectional", "BBS No Laplace Smoothing", "BBS Greedy"]
tool_filename_identifier = ["bbs_dbg_weight_output.txt", "bbs_unidirectional_output.txt", "bbs_no_laplace_smoothing_output.txt", "bbs_greedy_output.txt"]
bbs_success_rates = [0.9880, 0.9477, 0.9134]
...
for i in range(len(experiment_names)):
    ...
    for j, tool_name in enumerate(tool_names):
        ...
        success_rate_list.append(success_rate - bbs_success_rates[i])
...
sns.set_palette("muted", n_colors=len(tool_names))
