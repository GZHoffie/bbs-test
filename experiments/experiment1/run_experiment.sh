...
clusters_list=("../datasets/our_nanopore_UnderlyingClusters_subsampled.txt" 
               "../datasets/oligo0_UnderlyingClusters.txt" 
               "../datasets/Clusters_removed_empty_cluster.txt")
centers_list=("../datasets/our_nanopore_refs_subsampled.txt" 
...
do
    ...
    python3 run_all.py -i ${clusters_list[i]} -c ${centers_list[i]} -l ${read_lengths[i]} -o ${output_directory[i]} -s ${separator[i]} -b ${substitution_rate[i]} -d ${deletion_rate[i]} -n ${insertion_rate[i]}
done